...
int recd [nn], FECdata [kk], bb [nn-kk] ;
...
void encode_rs()
/* take the string of symbols in FECdata[i], i=0..(k-1) and encode systematically
   to produce 2*tt parity symbols in bb[0]..bb[2*tt-1]
   FECdata[] is input and bb[] is output in polynomial form.
   Encoding is done by using a feedback shift register with appropriate
   connections specified by the elements of gg[], which was generated above.
   Codeword is   c(X) = FECdata(X)*X**(nn-kk)+ b(X)          */
 {
   register int i,j ;
   int feedback ;
   ...
   for (i=kk-1; i>=0; i--)
    {  feedback = index_of[FECdata[i]^bb[nn-kk-1]] ;
       if (feedback != -1)
        { for (j=nn-kk-1; j>0; j--)
            if (gg[j] != -1)
...
/* for known FECdata, stick a few numbers into a zero codeword. FECdata is in
   polynomial form.
*/
for  (i=0; i<kk; i++)   FECdata[i] = 0 ;
...
/* encode FECdata[] to produce parity in bb[].  FECdata input and parity output
   is in polynomial form
*/
  encode_rs() ;

/* put the transmitted codeword, made up of FECdata plus parity, in recd[] */
  for (i=0; i<nn-kk; i++)  recd[i] = bb[i] ;
  for (i=0; i<kk; i++) recd[i+nn-kk] = FECdata[i] ;
...
/* print out the relevant stuff - initial and decoded {parity and message} */
  ...
  printf("  i  FECdata[i]   recd[i](decoded)   (FECdata, recd in polynomial form)\n");
  for (i=0; i<nn-kk; i++)
    printf("%3d    %3d      %3d\n",i, bb[i], recd[i]) ;
  for (i=nn-kk; i<nn; i++)
    printf("%3d    %3d      %3d\n",i, FECdata[i-nn+kk], recd[i]) ;
}
